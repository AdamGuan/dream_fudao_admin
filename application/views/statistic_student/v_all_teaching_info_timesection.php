...
<thead>
<tr>
	<th>学生账号</th><th>学生名字</th><th>年级</th><th>科目</th><th>老师账号</th><th>老师名字</th>
</tr>
</thead>
<tbody>
<?php if(isset($result['list'])) foreach($result['list'] as $k=>$item){
	$str = '';
	$str .= '<tr>';
	$str .= '<td>'.$item['F_student_name'].'</td>';
	$str .= '<td>'.$item['F_student_real_name'].'</td>';
	$str .= '<td>'.$item['F_grade_text'].'</td>';
	$str .= '<td>'.$item['F_subject_text'].'</td>';
	$str .= '<td>'.$item['F_teacher_name'].'</td>';
	$str .= '<td>'.$item['F_teacher_real_name'].'</td></tr>';
	echo $str;
}?>
</tbody>
</table>
