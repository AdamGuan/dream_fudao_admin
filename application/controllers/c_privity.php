...
class C_privity extends MY_Controller {
	 ...
	 * @param array $parames
	 */
	public function user_edit($parames = array()){
		//检查是否有登录
		$result = $this->_check_login();
		if(is_array($result) && isset($result['redirect_url']))	//未登录
		{
		...
		//业务
		$this -> load -> model('M_privity', 'mprivity');
		$user_info = $this->mprivity->get_user_info($parames);
		$group_list = $this->mprivity->get_group_list_valid($parames);
		$group_list = build_privity_group($group_list);

		//data
		$data = $this->_get_data(__CLASS__,__METHOD__);
		$data['content_title'] = "编辑工作人员";
		$data['user_info'] = $user_info;
		$data['group_list'] = $group_list;
		$this->_output_view("privity/v_user_edit", $data);
	}

	//ajax
	public function user_modify_do($parames = array()){
		//检查是否有登录
		$result = $this->_check_login();
		if(is_array($result) && isset($result['redirect_url']))	//未登录
		{
		...
		//业务
		$this -> load -> model('M_privity', 'mprivity');
		$data = $this->mprivity->user_update($parames);
